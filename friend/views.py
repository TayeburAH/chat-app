...
def send_friend_request(request, *args, **kwargs):
    ...
    if request.method == "POST" and user.is_authenticated:
        ...
        if user_id:
            ...
            try:
                # Get any friend requests (active and not-active), since there is no Unique in this table,
                # same requests will be stores again and again
                ...
                # when there are no previous friend requests so create one.
                friend_request = FriendRequest(sender=user, receiver=receiver)
                friend_request.save()
                payload['response'] = "Friend request sent."
...
def view_all_friend_requests(request, *args, **kwargs):
    context = {}
    user = request.user
    if user.is_authenticated:
        ...
        redirect("login")
    return render(request, "friend/view_friend_requests.html", context)
